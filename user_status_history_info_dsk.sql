...
'US','AG',	'AI',	'AW',	'BB',	'BQ','BZ','CW','KY','DM',	'GD','JM','KN',	'LC',	'MS',	'PR','SX','TT',	'VC'
    ...
    case when base_4.region = 'eu' then (
      CASE
      -- 1. A
      WHEN user_semester >= 14 AND (profession != 'physician' or profession is null) THEN
        CASE
         WHEN user_semester <=17 THEN (select label from `business-intelligence-194510.miamed_bi_test_all.occupation` where name = 'doctor t1')
         WHEN user_semester <=19 THEN (select label from `business-intelligence-194510.miamed_bi_test_all.occupation` where name = 'doctor t2')
         WHEN user_semester <=21 THEN (select label from `business-intelligence-194510.miamed_bi_test_all.occupation` where name = 'doctor t3')
         WHEN user_semester <=23 THEN (select label from `business-intelligence-194510.miamed_bi_test_all.occupation` where name = 'doctor t4')
         WHEN user_semester <=15 THEN (select label from `business-intelligence-194510.miamed_bi_test_all.occupation` where name = 'doctor t5')
         WHEN user_semester <=19 THEN (select label from `business-intelligence-194510.miamed_bi_test_all.occupation` where name = 'doctor t6')
         else (select label from `business-intelligence-194510.miamed_bi_test_all.occupation` where name = 'specialist')
      -- 2. A-label
       END
     END
    ) END
       as occupation_detection,
    user_semester
  FROM base_4
),
graduation_de as (
